...
class Node(object):
    def __init__(self, x, y) -> None:
        ...
        self.access = {
            ...
            STOP: [PACMAN, BLINKY, PINKY, INKY, CLYDE, FRUIT],
        }
...
class NodeGroup(object):
    ...
    def connectVertically(self, data, xoffest=0, yoffeset=0):
        ...
        for col in list(range(dataT.shape[0])):
            ...
            for row in list(range(dataT.shape[1])):
                if dataT[col][row] in self.nodeSymbols:
                    if key is None:
                        key = self.constructKey(col + xoffest, row + yoffeset)
                    else:
                        otherkey = self.constructKey(col + xoffest, row + yoffeset)
